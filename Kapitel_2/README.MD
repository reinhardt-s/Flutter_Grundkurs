...
* Feed Me eine App, die uns hilft, uns für ein Restaurant zu entscheiden  
![](feed_me_app.png)  
...
* Tausche den Inhalt des Container-Widgets gegen folgende Struktur aus:
  ...
  * Setze ein `SafeArea`-Widget um das `Center`-Widget  
...
1. Die AppBar hat den Titel "Holiday Inc.".
...
2. Der Body sollte mindestens wie folgt aussehen:
   - Eine SafeArea
   - Ein Scaffold in der SafeArea
   - Der `body` des `Scaffold`beginnt mit einem `Padding` von 16.0
